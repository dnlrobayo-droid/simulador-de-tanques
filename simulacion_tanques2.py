...
def generar_graficos_streamlit(historial, titulo_extra=""):
    ...
    if any(h['agua_carrotanques'] > 0 for h in historial):
        ax2.bar(horas, [h['agua_carrotanques'] for h in historial if h['hora'] > 0], label='Carrotanques', color='orange', alpha=0.7,
...
tab1, tab2, tab3 = st.tabs(["📊 Flujos", "⚡ Bombas", "🚚 Carrotanques"])  # CORREGIDO
...
with tab3:
    st.subheader("Configuración de Carrotanques")  # CORREGIDO
    usar_carrotanques = st.checkbox("Usar carrotanques")  # CORREGIDO
